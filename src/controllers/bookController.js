  ...
  try {
    ...
    if (!/^[a-zA-Z \s]+$/.test(category)) {
      return res
        ...
        .send({
          status: false,
          msg: "Please Enter Only Alphabets in Category",
        });
    }
    ...
    if (userId) {
      ...
      if (!findById) {
        res
          ...
          .send({
            status: false,
            message: "No user have books  with this id ",
          });
      }
    }
    if (category) {
...
const getBookById = async (req, res) => {
  try {
    ...
    if (!bookData)
      return res
        .status(404)
        .send({ status: false, message: "No Book found with that id" });
    let reviews = await reviewModel
      .find({ bookId: id, isDeleted: false })
      .select({ _id: 0, __v: 0 });
...
const updateBook = async (req, res) => {
  try {
    ...
    let bookData = await bookModel
      .findById(id)
      .select({ ISBN: 0, deletedAt: 0, __v: 0 });
    ...
    if (bookData["isDeleted"] == true)
      return res
        .status(400)
        .send({ status: false, message: "Book has been deleted" });

    let userIdFromBook = bookData.userId.toString();
    if (userIdFromBook !== req.userId)
      return res
        .status(403)
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          msg: "please enter require data to Update Book",
        });
    }
      ...
      if (checkTitle)
        return res
          ...
          .send({
            status: false,
            message:
              "There is a book with that title exists, please give another title",
          });
    }
    ...
    if (ISBN) {
      if (!/^\+?([1-9]{3})\)?[-. ]?([0-9]{10})$/.test(ISBN))
        return res
          .status(400)
          .send({ status: false, message: "Please enter a valid ISBN number" });
      ...
      if (checkISBN)
        return res
          ...
          .send({
            status: false,
            message:
              "There is a book with that ISBN exists, please give another ISBN",
          });
    }
